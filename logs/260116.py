...
def log_event(event_type, obj, func, extra_info=""):
    ...
    try:
        ...
        if hasattr(obj.Class, 'GetFullName'):
            ...
            # Fallback với Outer property (fix lỗi GetOuter)
            names = []
            current = obj.Class
            while current:
                names.append(str(current.Name))
                if hasattr(current, 'Outer'):
                    current = current.Outer
                else:
                    break  # Dừng nếu không có Outer
            names.reverse()
            obj_class = '/'.join(names) if names else "Unknown Class"
...
# ====================
# BL3_MOD_REFERENCE SYSTEM
# ====================

# Paths cho BL3_Mod_Reference
REF_DIR = Path(__file__).parent / "BL3_Mod_Reference"
CLASSES_FILE = REF_DIR / "sdk" / "classes.json"
EVENTS_FILE = REF_DIR / "events" / "inventory_events.json"
CONFIG_FILE = REF_DIR / "config.json"

# Tạo thư mục/files nếu chưa có
try:
    (REF_DIR / "sdk").mkdir(parents=True, exist_ok=True)
    (REF_DIR / "events").mkdir(parents=True, exist_ok=True)
    ...
    if not CONFIG_FILE.exists():
        CONFIG_FILE.write_text(json.dumps({
            ...
            "sources": ["bd3miner observed data", "apocalyptech/bl3data"],
            "description": "Thư viện tham chiếu BL3 modding"
        }, indent=2))
except Exception as e:
    write_log(f"Error creating BL3_Mod_Reference: {e}", "ERROR")

def save_to_db(class_name, path, event_type, extra=""):
    """Save observed data to BL3_Mod_Reference files"""
    try:
        # Load classes
        classes = {}
        ...
        if class_name not in classes:
            classes[class_name] = {
                "name": class_name,
                "path": path,
                "type": "pickup" if "Pickup" in class_name else "unknown",
                "category": "item" if "Item" in class_name else "unknown",
                "first_observed_at": datetime.now().isoformat(),
                "last_observed_at": datetime.now().isoformat(),
                "count": 1
            }
        else:
            classes[class_name]["last_observed_at"] = datetime.now().isoformat()
        ...
        # Add event (limit to 1000 entries)
        events.append({
        ...
        # Log summary (sạch, không chi tiết)
        write_log(f"Saved data for class '{class_name}' to BL3_Mod_Reference", "INFO")

    except Exception as e:
        write_log(f"Error saving to BL3_Mod_Reference: {e}", "ERROR")
                ...
                if hasattr(obj.Class, 'GetFullName'):
                    ...
                    while current:
                        ...
                        if hasattr(current, 'Outer'):
                            ...
                            break  # Dừng nếu không có Outer
                    names.reverse()
                    obj_name = '/'.join(names) if names else "Unknown"
...
def on_look_at_item(obj: UObject, args: WrappedStruct, ret: Any, func: BoundFunction):
    ...
    try:
            ...
            if hasattr(obj.Class, 'GetFullName'):
                ...
                while current:
                    ...
                    if hasattr(current, 'Outer'):
                        ...
                        break  # Dừng nếu không có Outer
                names.reverse()
                full_class_name = '/'.join(names) if names else "Unknown Class"
        ...
        # Save to BL3_Mod_Reference if path valid
        if full_class_name != "Unknown Class" and obj_name != "Unknown":
            save_to_db(obj_name, full_class_name, "ITEM_LOOKED_AT", f"Visible name: {visible_name}")
...
def on_use_object(obj: UObject, args: WrappedStruct, ret: Any, func: BoundFunction):
    ...
    try:
        ...
        # Save to BL3_Mod_Reference if path valid
        if full_class_name != "Unknown Class" and obj_name != "Unknown Object":
            save_to_db(obj_name, full_class_name, "OBJECT_USED", f"Type: {obj_type}")
...
mod = build_mod(
    name="bd3miner",
    author="User & AI",
    description="Item/Object Class ID Scanner with ProcessEvent hooks and BL3_Mod_Reference library. Log: " + str(LOG_FILE),
    version=__version__
)
...
write_log("", "INFO")
write_log("Data is saved to BL3_Mod_Reference/ for library use.", "INFO")
